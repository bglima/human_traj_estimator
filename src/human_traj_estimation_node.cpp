...
int main(int argc, char **argv)
{
  ...
  ros::Publisher human_applied_filtered_wrench_pub = nh.advertise<geometry_msgs::WrenchStamped>("/human_applied_filtered_wrench_in_base_frame", 10);
...
//   ros::ServiceServer update_Kest_server = nh.advertiseService(update_Kest, &TrajEstimator::updateKestSrv, &te);
  ...
  while (ros::ok())
  {
    ...
    geometry_msgs::WrenchStamped human_applied_filtered_wrench = te.getCurrentHummanAppliedFilteredWrenchUnbiased();
    ...
    else
    {
      ...
      human_applied_filtered_wrench_pub.publish(human_applied_filtered_wrench);
